...
public class network {
	...
	public static class MessageThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				while ((input = readerIn.readLine()) != null) {
					...
					// If corrupt, corrupt the checksum, generate the message and send to other
					// thread
					else if (x >= 0.5 && x <= 0.75) // CORRUPT
					{
						// System.out.println(input);
						helper_class p = new helper_class();
		...
		// This method keeps track of which thread it is. It sends to the opposite
		// thread.
