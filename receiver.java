...
public class receiver {
	...
	public static void main(String[] args) {
		if (args.length != 2) {
			System.err.println("Error: java receiver <host name> <port number>");
			System.exit(1);
		}
	...
	public void startReceiver(String hostName, int portNumber) throws IOException {
		...
		try {
			...
			while ((input = bufferIn.readLine()) != null) {
				...
				msg = msg + pak.content + " ";
				totalReceived++;
				// Print out current sequence number, total packets received, message, and ACK
				// to be transmitted
				output = "Waiting " + pak.seqNo + ", " + totalReceived + ", " + input + ", " + pak.validateMessage();
				System.out.println(output);
				// Validate message (from packet object) - checks the checksum and then returns
				// the proper ACK
