...
public class sender {
	...
	public static void main(String[] args) {
		if (args.length != 3) {
			System.err.println("Error: java sender <host name> <port number> <message file name>");
			System.exit(1);
		}
	...
	public void startSender(String hostName, int portNumber, String fileName) throws IOException {
		...
		try {
			...
			try {
				...
				helper_class pak = new helper_class();
				while ((input = buffReader.readLine()) != null) {
					...
					while (i < splited.length) {
						...
						if (ACK.equals("ACK2")) { // DROP
							System.out.println(
									"Waiting: " + ACK + ", " + totalSent + ", DROP, resend Packet" + pak.seqNo);
						} else if (validateACK(ACK, pak.seqNo) == true) { // PASS
							i++;
							System.out.println(
									"Waiting: " + ACK + ", " + totalSent + ", " + ACK + ", no more packets to send");
						} else { // CORRUPT
							System.out.println(
									"Waiting: " + ACK + ", " + totalSent + ", " + ACK + ", send Packet" + pak.seqNo);
						}
					}
